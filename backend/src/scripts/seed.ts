...
import { CreateCountryGeoZoneDTO, ExecArgs } from "@medusajs/types";
import {
...
export default async function seedDemoData({ container }: ExecArgs) {
  ...
  for (const regionDef of REGIONS) {
    ...
    const serviceZone = {
      ...
      // map only required fields for country geo zone
      geo_zones: regionDef.countries.map((code): Omit<CreateCountryGeoZoneDTO, "service_zone_id"> => ({ country_code: code, type: "country" })),
    };
